...
function setParticlesOptionsFillColor(particlesOptions, color, opacity, enable) {
  ...
  paint.fill = {
    color: AnimatableColor.create(void 0, {
      value: color
    }),
    enable,
    opacity
...
var EmitterInstance = class {
  ...
  #emitParticles(quantity) {
    const singleParticlesOptions = itemFromSingleOrMultiple(this.#particlesOptions) ?? {}, fillHslAnimation = this.options.spawn.fill?.color?.animation, fillEnabled = this.options.spawn.fill?.enable ?? !!this.options.spawn.fill?.color, fillOpacity = this.options.spawn.fill?.opacity === void 0 ? defaultOpacity : getRangeValue(this.options.spawn.fill.opacity), strokeHslAnimation = this.options.spawn.stroke?.color?.animation, strokeOpacity = this.options.spawn.stroke?.opacity === void 0 ? defaultOpacity : getRangeValue(this.options.spawn.stroke.opacity), strokeWidth = this.options.spawn.stroke?.width === void 0 ? defaultStrokeWidth : getRangeValue(this.options.spawn.stroke.width), reduceFactor = this.#container.retina.reduceFactor, needsFillColorAnimation = !!fillHslAnimation, needsStrokeColorAnimation = !!strokeHslAnimation, needsShapeData = !!this.#shape, needsColorAnimation = needsFillColorAnimation || needsStrokeColorAnimation, needsCopy = needsColorAnimation || needsShapeData, maxValues = needsColorAnimation ? {
      h: hMax,
      s: sMax,
      l: lMax
  ...
  #prepareToDie() {
    ...
    if ((this.#lifeCount > minLifeCount2 || this.#immortal) && duration !== void 0 && duration > minDuration) {
...
//# sourceMappingURL=EmitterInstance-WX2HNPHG.js.map
