...
var RenderManager = class {
  ...
  drawParticle(particle, delta) {
    ...
    const container = this.#container, zIndexOptions = particle.options.zIndex, zIndexFactor = zIndexFactorOffset - particle.zIndexFactor, {
      ...
      opacity,
      strokeOpacity
    } = particle.getOpacity(), transform = this.#reusableTransform, colorStyles = this.#reusableColorStyles, fill = fColor ? getStyleFromHsl(fColor, container.hdr, fillOpacity * opacity) : void 0, stroke = sColor ? getStyleFromHsl(sColor, container.hdr, strokeOpacity * opacity) : fill;
  ...
  #drawParticle(data) {
    const {
      ...
      effectDrawers,
      shapeDrawers
    } = container, pos = particle.getPosition(), transformData = particle.getTransformData(transform), drawScale = defaultZoom, drawPosition = {
      x: pos.x,
      y: pos.y
    };
...
var CanvasManager = class {
  ...
  #initContext() {
    ...
    this.render.setContextSettings(__spreadValues({
      alpha: true,
      desynchronized: true,
...
var Particle = class {
  ...
  getOpacity() {
    const zIndexOptions = this.options.zIndex, zIndexFactor = zIndexFactorOffset - this.zIndexFactor, zOpacityFactor = zIndexFactor ** zIndexOptions.opacityRate, baseOpacity = getRangeValue(this.opacity?.value ?? defaultOpacity), modifierOpacity = this.#applyModifiers(void 0, (m) => m.opacity), opacity = modifierOpacity ?? baseOpacity, fillOpacity = this.fillOpacity ?? defaultOpacity, strokeOpacity = this.strokeOpacity ?? defaultOpacity;
...
var SpatialHashGrid = class {
  ...
  insert(particle) {
    const {
      x,
      y
    } = particle.getPosition(), key = this.#cellKeyFromCoords(x, y);
...
var ParticlesManager = class {
  ...
  init() {
    return __async(this, null, function* () {
      ...
      yield this.#container.initDrawersAndUpdaters();
      for (const drawer of this.#container.effectDrawers.values()) {
        yield drawer.init?.(container);
      }
...
var Container = class {
  ...
  init() {
    return __async(this, null, function* () {
      ...
      for (const plugin of this.#pluginManager.plugins) {
        const containerPlugin = yield plugin.getPlugin(this);
        if (containerPlugin.preInit) {
          yield containerPlugin.preInit();
        }
        allContainerPlugins.set(plugin, containerPlugin);
      }
      yield this.initDrawersAndUpdaters();
  ...
  initDrawersAndUpdaters() {
    return __async(this, null, function* () {
      const pluginManager = this.#pluginManager;
      this.effectDrawers = yield pluginManager.getEffectDrawers(this, true);
      this.shapeDrawers = yield pluginManager.getShapeDrawers(this, true);
      this.particleUpdaters = yield pluginManager.getUpdaters(this, true);
    });
  }
  pause() {
    if (!guardCheck(this)) {
...
//# sourceMappingURL=Container-3TBAQAA5.js.map
